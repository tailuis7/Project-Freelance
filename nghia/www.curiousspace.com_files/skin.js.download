...
function dpPopulate(data) {
	...
	dealPlySkinObj.injectTranslations(dealPlySkinObj.start);
...
DealPlySkinCls.prototype.injectTranslations = function(callback) {
				...
				if (status != "success") {
					...
					dpQuery.getScript(url).done(function(script, status) {
						if (status == "success") {
							...
							dpVariables.strings = dealPlySkinObj.translations;
							callback();
						}
					});
